            ...
            <div className="flex items-center gap-2">
                ...
                <div className="w-[70%]">
                    <button onClick={() => setModal(true)} className="btn-white shadow-md">
                        <div className="text-black">{context.poli ? context.poli.name : `Pilih Poli `}</div>
                    </button>
                </div>
            </div>
                            ...
                            {context.dataPoli && context.dataPoli.map((item: any, index: number) => {
                                return (
                                    <React.Fragment key={index}>
                                        <button onClick={() => handleModal(item)} className={`${context.poli && context.poli.id == item.id ? `btn-primary` : `btn-white`} shadow-md aspect-square`}>
                                            {item.name}
                                        </button>
                                    </React.Fragment>
                                )
